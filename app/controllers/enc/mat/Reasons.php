...
class Reasons extends \Controller {

   public function edit() {
                $this->f3->set('breadcrumbs','enc/mat/reasons');
                ...
                if (strpos($rids,",")>0) {
                        ...
                        $record  = $this->enc->exec('SELECT rid,Epoch,PartNumber,Description,DueDate,paretoReason as reason FROM enc_matlog WHERE rid = ?',$this->f3->get('PARAMS.id'));
                }
                $this->f3->set('mode','upd');
                $this->f3->set('epoch',$rids);
                $this->f3->set('record',$record);
                $this->f3->set('view','enc/mat/buyers/reasons.htm');
    }
    public function upd() {
