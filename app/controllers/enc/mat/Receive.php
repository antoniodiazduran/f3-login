...
public function edit() {
                $this->f3->set('breadcrumbs','enc/mat/receive');
                $rids = $this->f3->get('PARAMS.id');
                if (strpos($rids,",")>0) {
                        $rids = rtrim($rids,",");
                        $sqlstrm = 'SELECT rid,Epoch,PartNumber,Description,DueDate,paretoReason,ArrivedDate as reason FROM enc_matlog WHERE rid in ('.$rids.')';
                        $record  = $this->enc->exec($sqlstrm);
                } else {
                        $record  = $this->enc->exec('SELECT rid,Epoch,PartNumber,Description,DueDate,paretoReason,ArrivedDate as reason FROM enc_matlog WHERE rid = ?',$this->f3->get('PARAMS.id'));
                }
                $this->f3->set('mode','upd');
                $this->f3->set('epoch',$rids);
                $this->f3->set('record',$record);
                $this->f3->set('view','enc/mat/buyers/receive.htm');
    }
    public function upd() {
